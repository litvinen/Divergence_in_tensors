...
K = -0.5 * Temp2;
[U eS1 V] = svd(S1);
Temp3 = sum(log(diag(eS1)))*r/2;
[U eS2 V] = svd(S2);
Temp4 = sum(log(diag(eS2)))*(1-r)/2;
...
fun_P_log = @(x)((-sum((x-mu1(:)').^2,2)/(2*sigm1^2)) -(d/2)*log(2*pi) - 0.5*sum(log(diag(eS1))));
fun_P = @(x)exp(fun_P_log(x));
%fun_P = @(x)exp(-sum((x-mu1(:)').^2,2)/(2*sigm1^2))/ ((2*pi)^(d/2)*sqrt(detS1));
fun_Q_log = @(x)((-sum((x-mu2(:)').^2,2)/(2*sigm2^2)) -(d/2)*log(2*pi) - 0.5*sum(log(diag(eS2))));
fun_Q = @(x)exp(fun_Q_log(x));
...
funP_sqHellinger = @(x) (sqrt(fun_P(x)))
funQ_sqHellinger = @(x) (sqrt(fun_Q(x)))

tic
%sqHellinger_tensor = amen_cross_s(n*ones(d,1), @(ind)fun_sqHellinger (reshape(x(ind), [], d)), tol, 'vec',true, 'y0', g0);
P_tensor = amen_cross_s(n*ones(d,1), @(ind)funP_sqHellinger (reshape(x(ind), [], d)), tol, 'vec',true, 'y0', g0);
Q_tensor = amen_cross_s(n*ones(d,1), @(ind)funQ_sqHellinger (reshape(x(ind), [], d)), tol, 'vec',true, 'y0', g0);
D=P_tensor - Q_tensor;
...
%sqHellinger_value = dot(tt_ones(n,d)*h^d, D)
sqHellinger_value = dot(D, D)*h^d
Hellinger_TT = sqrt(sqHellinger_value/2)
comptime = toc
