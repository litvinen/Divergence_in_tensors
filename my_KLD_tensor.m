...
tol = 1e-12;
alpha=2.0; % working for alpha={0.7, 1.0, 1.5, 2.0}
mu = 0.0.*(1:d)';
...
fun_KLD = @(x) x(:,1).*(log(x(:,1)) - log(x(:,2)));
KLD_cross = amen_cross_s({f_P, f_Q}, fun_KLD, tol);
display(KLD_cross)
dt=whos('KLD_cross'); 
MB=dt.bytes*9.53674e-7

% Compute KLD
KLD_value = real(dot(Id, KLD_cross))% *(2*pi/(x(2)-x(1)))^d 
